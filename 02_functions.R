...
# Isaric minimal -------------------------------------------------------------------------
isaric_min <- function(.data, age, sex, comorbid, rr, spo2, gcs, crp, bun,
                       # nlr, plts, hb, bun, creat, crp, lactate,
                       output = c("vector", "components", "df_vector", "df_components"),
                       na_to_zeros = TRUE, all_na_rm = TRUE){
  ...
  out = .data %>%
    ...
    mutate(
      ...
      isaric_min_sex = case_when(
        !! .sex == "male" ~ 1,
        !! .sex == "female" ~ 0,
        TRUE ~ NA_real_),

      isaric_min_comorbid = case_when(
        !! .comorbid == "0" ~ 0,
        !! .comorbid == "1" ~ 1,
        !! .comorbid >= "2" ~ 3,
        TRUE ~ NA_real_),

      isaric_min_rr = case_when(
        !! .rr < 20 ~ 0,
        !! .rr < 30 ~ 1,
        !! .rr >= 30 ~ 2,
        TRUE ~ NA_real_),

      isaric_min_spo2 = case_when(
        !! .spo2 < 90 ~ 4,
        !! .spo2 >= 90 ~ 0,
        TRUE ~ NA_real_),

      isaric_min_gcs = case_when(
        !! .gcs <=  14 ~ 5,
        !! .gcs > 14 ~ 0,
        TRUE ~ NA_real_),

      # isaric_min_nlr = case_when(
      #   !! .nlr <= 6 ~ 0,
      #   !! .nlr >  6 ~ 1,
      #   TRUE ~ NA_real_),

      # isaric_min_plts = case_when(
      #   !! .plts < 100 ~ 3,
      #   !! .plts >=100 ~ 0,
      #   TRUE ~ NA_real_),
      #
      # isaric_min_hb = case_when(
      #   !! .hb < 120 ~ 2,
      #   !! .hb >= 120 ~ 0,
      #   TRUE ~ NA_real_),
      #
      isaric_min_bun = case_when(
        !! .bun <= 7 ~ 0,
        !! .bun >  7 ~ 3,
        TRUE ~ NA_real_),
      #
      # isaric_min_creat = case_when(
      #   !! .creat < 100 ~ 0,
      #   !! .creat >= 100 ~ 2,
      #   TRUE ~ NA_real_),
      #
      isaric_min_crp = case_when(
        !! .crp < 100 ~ 0,
        !! .crp >=100 ~ 3,
        TRUE ~ NA_real_)
      #
      # isaric_min_lactate = case_when(
      #   !! .lactate < 2 ~ 0,
      #   !! .lactate >=2 ~ 2,
      #   TRUE ~ NA_real_)
    ...
    mutate(
      isaric_min = rowSums(dplyr::select(., dplyr::starts_with("isaric_min_")),
                           na.rm = na_to_zeros)
    ) %>%

    {if(all_na_rm){
      dplyr::mutate(., isaric_min = dplyr::if_else(
        is.na(isaric_min_age) &
          is.na(isaric_min_sex) &
          is.na(isaric_min_comorbid) &
          is.na(isaric_min_rr) &
          is.na(isaric_min_spo2) &
          is.na(isaric_min_gcs) &
          # is.na(isaric_min_nlr) &
          # is.na(isaric_min_plts) &
          # is.na(isaric_min_hb) &
          is.na(isaric_min_bun) &
          # is.na(isaric_min_creat) &
          # is.na(isaric_min_lactate),
          is.na(isaric_min_crp), NA_real_, isaric_min))
    } else {
      .
    }}
  ...
  if(output == "vector"){
    ...
    out %>%
      select(starts_with("isaric_min"))
  } else if(output == "df_vector"){
    out %>%
      pull(isaric_min) %>%
      bind_cols(.data, isaric_min = .)
  } else if(output == "df_components"){
    out
  }
}
...
# Isaric clinical -------------------------------------------------------------------------
isaric_clin <- function(.data, age, sex, comorbid, rr, spo2, gcs, # crp, bun,
                        output = c("vector", "components", "df_vector", "df_components"),
                        na_to_zeros = TRUE, all_na_rm = TRUE){
  ...
  out = .data %>%
    ...
    mutate(
      ...
      isaric_clin_sex = case_when(
        !! .sex == "male" ~ 1,
        !! .sex == "female" ~ 0,
        TRUE ~ NA_real_),

      isaric_clin_comorbid = case_when(
        !! .comorbid == "0" ~ 0,
        !! .comorbid == "1" ~ 1,
        !! .comorbid >= "2" ~ 3,
        TRUE ~ NA_real_),

      isaric_clin_rr = case_when(
        !! .rr < 20 ~ 0,
        !! .rr < 30 ~ 1,
        !! .rr >= 30 ~ 2,
        TRUE ~ NA_real_),

      isaric_clin_spo2 = case_when(
        !! .spo2 < 90 ~ 4,
        !! .spo2 >= 90 ~ 0,
        TRUE ~ NA_real_),

      isaric_clin_gcs = case_when(
        !! .gcs <=  14 ~ 4,
        !! .gcs > 14 ~ 0,
        TRUE ~ NA_real_)

      # isaric_clin_nlr = case_when(
      #   !! .nlr <= 6 ~ 0,
      #   !! .nlr >  6 ~ 1,
      #   TRUE ~ NA_real_),

      # isaric_clin_plts = case_when(
      #   !! .plts < 100 ~ 3,
      #   !! .plts >=100 ~ 0,
      #   TRUE ~ NA_real_),
      #
      # isaric_clin_hb = case_when(
      #   !! .hb < 120 ~ 2,
      #   !! .hb >= 120 ~ 0,
      #   TRUE ~ NA_real_),
      #
      # isaric_clin_bun = case_when(
      #   !! .bun <= 7 ~ 0,
      #   !! .bun >  7 ~ 2,
      #   TRUE ~ NA_real_),
      #
      # isaric_clin_creat = case_when(
      #   !! .creat < 100 ~ 0,
      #   !! .creat >= 100 ~ 2,
      #   TRUE ~ NA_real_),
      #
      # isaric_clin_crp = case_when(
      #   !! .crp < 100 ~ 0,
      #   !! .crp >=100 ~ 3,
      #   TRUE ~ NA_real_)
      #
      # isaric_clin_lactate = case_when(
      #   !! .lactate < 2 ~ 0,
      #   !! .lactate >=2 ~ 2,
      #   TRUE ~ NA_real_)
    ...
    mutate(
      isaric_clin = rowSums(dplyr::select(., dplyr::starts_with("isaric_clin_")),
                            na.rm = na_to_zeros)
    ) %>%

    {if(all_na_rm){
      dplyr::mutate(., isaric_clin = dplyr::if_else(
        is.na(isaric_clin_age) &
          is.na(isaric_clin_sex) &
          is.na(isaric_clin_comorbid) &
          is.na(isaric_clin_rr) &
          is.na(isaric_clin_spo2) &
          is.na(isaric_clin_gcs),
        # is.na(isaric_clin_nlr) &
        # is.na(isaric_clin_plts) &
        # is.na(isaric_clin_hb) &
        # is.na(isaric_clin_bun) &
        # is.na(isaric_clin_creat) &
        # is.na(isaric_clin_lactate),
        # is.na(isaric_clin_crp),
        NA_real_, isaric_clin))
    } else {
      .
    }}
  ...
  if(output == "vector"){
    ...
    out %>%
      select(starts_with("isaric_clin"))
  } else if(output == "df_vector"){
    out %>%
      pull(isaric_clin) %>%
      bind_cols(.data, isaric_clin = .)
  } else if(output == "df_components"){
    out
  }
}
...
# Lasso scores -------------------------------------------------------------------------------------------
lasso_score_full <- function(.data, age, sex, comorbid, rr, spo2, hr, sbp, gcs, infiltrates,
                             plts, nlr, bun, creat, crp,
                             output = c("vector", "components", "df_vector", "df_components")){
  ...
  out = .data %>%
    dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .spo2,
                          !! .hr, !! .sbp, !! .gcs, !! .plts, !! .nlr, !! .bun, !! .creat, !! .crp
    ), as.numeric) %>%
    dplyr::mutate_at(vars(!! .sex, !! .infiltrates), ~ as.character(.) %>% tolower() %>% trimws()) %>%

    mutate(
      # Lasso full score
      lasso_score_full_age = !! .age * 0.5390,
      lasso_score_full_comorbid = !! .comorbid * 0.1703,
      lasso_score_full_rr = !! .rr * 0.0201,
      lasso_score_full_spo2 = !! .spo2 * -0.0539,
      lasso_score_full_hr = !! .hr * 0.0044,
      lasso_score_full_sbp = !! .sbp * -0.0023,
      lasso_score_full_gcs = !! .gcs * -0.0958,
      lasso_score_full_plts = !! .plts * -0.0010,
      lasso_score_full_nlr = !! .nlr * 0.0025,
      lasso_score_full_bun = !! .bun * 0.0223,
      lasso_score_full_creat = !! .creat * 0.0005,
      lasso_score_full_crp = !! .crp * 0.0021,

      lasso_score_full_sex = case_when(
        !! .sex == "male" ~ 0.0371,
        is.na(!! .sex) ~ NA_real_,
        TRUE ~ 0),

      lasso_score_full_infiltrates = case_when(
        !! .infiltrates == "Yes" ~ 0.0310,
        is.na(!! .infiltrates) ~ NA_real_,
        TRUE ~ 0),

      lasso_score_full_link = 0.8557 + lasso_score_full_age + lasso_score_full_sex + lasso_score_full_comorbid + lasso_score_full_rr +
        lasso_score_full_spo2 + lasso_score_full_hr + lasso_score_full_sbp + lasso_score_full_gcs + lasso_score_full_plts +
        lasso_score_full_nlr + lasso_score_full_bun + lasso_score_full_creat + lasso_score_full_crp + lasso_score_full_infiltrates,
      lasso_score_full = boot::inv.logit((lasso_score_full_link))
    )

  if(output == "vector"){
    out %>%
      pull(lasso_score_full)
  } else if(output == "components"){
    out %>%
      select(starts_with("lasso_score_full"))
  } else if(output == "df_vector"){
    out %>%
      pull(lasso_score_full) %>%
      bind_cols(.data, lasso_score_full = .)
  } else if(output == "df_components"){
    out
  }
}
...
### lambda min
lasso_score_full2 <- function(.data, age, sex, comorbid, rr, spo2, hr, sbp, dbp, gcs, infiltrates,
                              plts, nlr, bun, creat, crp,
                              output = c("vector", "components", "df_vector", "df_components")){
  ...
  out = .data %>%
    dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .spo2,
                          !! .hr, !! .sbp, !! .dbp, !! .gcs, !! .plts, !! .nlr, !! .bun, !! .creat, !! .crp
    ), as.numeric) %>%
    dplyr::mutate_at(vars(!! .sex, !! .infiltrates), ~ as.character(.) %>% tolower() %>% trimws()) %>%

    mutate(
      # Lasso full score
      lasso_score_full2_age = !! .age * 0.6221,
      lasso_score_full2_comorbid = !! .comorbid * 0.2042,
      lasso_score_full2_rr = !! .rr * 0.0246,
      lasso_score_full2_spo2 = !! .spo2 * -0.0613,
      lasso_score_full2_hr = !! .hr * 0.0082,
      lasso_score_full2_sbp = !! .sbp * -0.0038,
      lasso_score_full2_dbp = !! .dbp * -0.0038,
      lasso_score_full2_gcs = !! .gcs * -0.1351,
      lasso_score_full2_plts = !! .plts * -0.0017,
      lasso_score_full2_nlr = !! .nlr * 0.0044,
      lasso_score_full2_bun = !! .bun *  0.0243,
      lasso_score_full2_creat = !! .creat * 0.0007,
      lasso_score_full2_crp = !! .crp * 0.0025,

      lasso_score_full2_sex = case_when(
        !! .sex == "male" ~ 0.1427,
        is.na(!! .sex) ~ NA_real_,
        TRUE ~ 0),

      lasso_score_full2_infiltrates = case_when(
        !! .infiltrates == "Yes" ~ 0.1454,
        is.na(!! .infiltrates) ~ NA_real_,
        TRUE ~ 0),

      lasso_score_full2_link = 1.304 + lasso_score_full2_age + lasso_score_full2_sex + lasso_score_full2_comorbid + lasso_score_full2_rr +
        lasso_score_full2_spo2 + lasso_score_full2_hr + lasso_score_full2_sbp + lasso_score_full2_dbp + lasso_score_full2_gcs + lasso_score_full2_plts +
        lasso_score_full2_nlr + lasso_score_full2_bun + lasso_score_full2_creat + lasso_score_full2_crp + lasso_score_full2_infiltrates,
      lasso_score_full2 = boot::inv.logit((lasso_score_full2_link))
    )

  if(output == "vector"){
    out %>%
      pull(lasso_score_full2)
  } else if(output == "components"){
    out %>%
      select(starts_with("lasso_score_full2"))
  } else if(output == "df_vector"){
    out %>%
      pull(lasso_score_full2) %>%
      bind_cols(.data, lasso_score_full2 = .)
  } else if(output == "df_components"){
    out
  }
}
...
# Isaric_cox function ----
isaric_cox <- function(.data, age, sex, comorbid, rr, spo2, gcs, crp, bun,
                       # nlr, plts, creat,
                       output = c("vector", "components", "df_vector", "df_components"),
                       na_to_zeros = TRUE, all_na_rm = TRUE){
  ...
  out = .data %>%
    ...
    mutate(
      ...
      isaric_cox_sex = case_when(
        !! .sex == "male" ~ 0,
        !! .sex == "female" ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_comorbid = case_when(
        !! .comorbid == "0" ~ 0,
        !! .comorbid == "1" ~ 1,
        !! .comorbid >= "2" ~ 2,
        TRUE ~ NA_real_),

      isaric_cox_rr = case_when(
        !! .rr < 30 ~ 0,
        !! .rr >= 30 ~ 2,
        TRUE ~ NA_real_),

      isaric_cox_spo2 = case_when(
        !! .spo2 < 90 ~ 3,
        !! .spo2 >= 90 ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_gcs = case_when(
        !! .gcs <=  14 ~ 3,
        !! .gcs > 14 ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_bun = case_when(
        !! .bun <= 7 ~ 0,
        !! .bun >  7 ~ 3,
        TRUE ~ NA_real_),

      isaric_cox_crp = case_when(
        !! .crp < 100 ~ 0,
        !! .crp >=100 ~ 2,
        TRUE ~ NA_real_),

      # isaric_cox_nlr = case_when(
      #   !! .nlr <= 6 ~ 0,
      #   !! .nlr >  6 ~ 1,
      #   TRUE ~ NA_real_),
      #
      # isaric_cox_plts = case_when(
      #   !! .plts < 100 ~ 1,
      #   !! .plts >=100 ~ 0,
      #   TRUE ~ NA_real_),
      #
      # isaric_cox_creat = case_when(
      #   !! .creat < 100 ~ 0,
      #   !! .creat >= 100 ~ 2,
      #   TRUE ~ NA_real_),
    ...
    mutate(
      isaric_cox = rowSums(dplyr::select(., dplyr::starts_with("isaric_cox_")),
                           na.rm = na_to_zeros)
    ) %>%

    {if(all_na_rm){
      dplyr::mutate(., isaric_cox = dplyr::if_else(
        is.na(isaric_cox_age) &
          is.na(isaric_cox_sex) &
          is.na(isaric_cox_comorbid) &
          is.na(isaric_cox_rr) &
          is.na(isaric_cox_spo2) &
          is.na(isaric_cox_gcs) &
          # is.na(isaric_cox_nlr) &
          # is.na(isaric_cox_plts) &
          # is.na(isaric_cox_hb) &
          is.na(isaric_cox_bun) &
          # is.na(isaric_cox_creat) &
          # is.na(isaric_cox_lactate),
          is.na(isaric_cox_crp), NA_real_, isaric_cox))
    } else {
      .
    }}
  ...
  if(output == "vector"){
    ...
    out %>%
      select(starts_with("isaric_cox"))
  } else if(output == "df_vector"){
    out %>%
      pull(isaric_cox) %>%
      bind_cols(.data, isaric_cox = .)
  } else if(output == "df_components"){
    out
  }
}
...
# Isaric_cox_clinical function ----
isaric_cox_clinical <- function(.data, age, sex, comorbid, rr, spo2, gcs,
                                # nlr, plts, creat,
                                output = c("vector", "components", "df_vector", "df_components"),
                                na_to_zeros = TRUE, all_na_rm = TRUE){
  ...
  out = .data %>%
    ...
    mutate(
      isaric_cox_clinical_age = case_when(
        !! .age < 50 ~ 0,
        !! .age < 60 ~ 0,
        !! .age < 70 ~ 2,
        !! .age < 80 ~ 4,
      ...
      isaric_cox_clinical_sex = case_when(
        !! .sex == "male" ~ 0,
        !! .sex == "female" ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_clinical_comorbid = case_when(
        !! .comorbid == "0" ~ 0,
        !! .comorbid == "1" ~ 1,
        !! .comorbid >= "2" ~ 2,
        TRUE ~ NA_real_),

      isaric_cox_clinical_rr = case_when(
        !! .rr < 30 ~ 0,
        !! .rr >= 30 ~ 2,
        TRUE ~ NA_real_),

      isaric_cox_clinical_spo2 = case_when(
        !! .spo2 < 90 ~ 3,
        !! .spo2 >= 90 ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_clinical_gcs = case_when(
        !! .gcs <=  14 ~ 3,
        !! .gcs > 14 ~ 0,
        TRUE ~ NA_real_),

      # isaric_cox_clinical_bun = case_when(
      #   !! .bun <= 7 ~ 0,
      #   !! .bun >  7 ~ 3,
      #   TRUE ~ NA_real_),
      #
      # isaric_cox_clinical_crp = case_when(
      #   !! .crp < 100 ~ 0,
      #   !! .crp >=100 ~ 2,
      #   TRUE ~ NA_real_),

      # isaric_cox_clinical_nlr = case_when(
      #   !! .nlr <= 6 ~ 0,
      #   !! .nlr >  6 ~ 1,
      #   TRUE ~ NA_real_),
      #
      # isaric_cox_clinical_plts = case_when(
      #   !! .plts < 100 ~ 1,
      #   !! .plts >=100 ~ 0,
      #   TRUE ~ NA_real_),
      #
      # isaric_cox_clinical_creat = case_when(
      #   !! .creat < 100 ~ 0,
      #   !! .creat >= 100 ~ 2,
      #   TRUE ~ NA_real_),
    ...
    mutate(
      isaric_cox_clinical = rowSums(dplyr::select(., dplyr::starts_with("isaric_cox_clinical_")),
                                    na.rm = na_to_zeros)
    ) %>%

    {if(all_na_rm){
      dplyr::mutate(., isaric_cox_clinical = dplyr::if_else(
        is.na(isaric_cox_clinical_age) &
          is.na(isaric_cox_clinical_sex) &
          is.na(isaric_cox_clinical_comorbid) &
          is.na(isaric_cox_clinical_rr) &
          is.na(isaric_cox_clinical_spo2) &
          is.na(isaric_cox_clinical_gcs),
        # is.na(isaric_cox_clinical_nlr) &
        # is.na(isaric_cox_clinical_plts) &
        # is.na(isaric_cox_clinical_hb) &
        # is.na(isaric_cox_clinical_bun) &
        # is.na(isaric_cox_clinical_creat) &
        # is.na(isaric_cox_clinical_lactate),
        # is.na(isaric_cox_clinical_crp
        NA_real_, isaric_cox_clinical))
    } else {
      .
    }}
  ...
  if(output == "vector"){
    out %>%
      pull(isaric_cox_clinical)
  } else if(output == "components"){
    out %>%
      select(starts_with("isaric_cox_clinical"))
  } else if(output == "df_vector"){
    out %>%
      pull(isaric_cox_clinical) %>%
      bind_cols(.data, isaric_cox_clinical = .)
  } else if(output == "df_components"){
    out
  }
}
...
# Isaric_cox_average function ----
isaric_cox_average <- function(.data, age, sex, comorbid, rr, spo2, gcs, crp, bun, #plts,
                               # nlr, creat,
                               output = c("vector", "components", "df_vector", "df_components"),
                               na_to_zeros = TRUE, all_na_rm = TRUE){
  ...
  out = .data %>%
    dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .spo2, !! .gcs, !! .crp, !! .bun, #!! .plts
                          # !! .nlr, !! .hb, !! .bun, !! .creat, !! .crp, !! .lactate
    ), as.numeric) %>%
    dplyr::mutate_at(vars(!! .sex), ~ as.character(.) %>% tolower() %>% trimws()) %>%

    mutate(
      isaric_cox_average_age = case_when(
        !! .age < 50 ~ 0,
        !! .age < 60 ~ 0,
        !! .age < 70 ~ 2,
        !! .age < 80 ~ 5,
      ...
      isaric_cox_average_sex = case_when(
        !! .sex == "male" ~ 0,
        !! .sex == "female" ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_average_comorbid = case_when(
        !! .comorbid == "0" ~ 0,
        !! .comorbid == "1" ~ 0,
        !! .comorbid >= "2" ~ 1,
        TRUE ~ NA_real_),

      isaric_cox_average_rr = case_when(
        !! .rr < 30 ~ 0,
        !! .rr >= 30 ~ 2,
        TRUE ~ NA_real_),

      isaric_cox_average_spo2 = case_when(
        !! .spo2 < 90 ~ 3,
        !! .spo2 >= 90 ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_average_gcs = case_when(
        !! .gcs <=  14 ~ 3,
        !! .gcs > 14 ~ 0,
        TRUE ~ NA_real_),

      isaric_cox_average_bun = case_when(
        !! .bun <= 7 ~ 0,
        !! .bun >  7 ~ 3,
        TRUE ~ NA_real_),

      isaric_cox_average_crp = case_when(
        !! .crp < 100 ~ 0,
        !! .crp >=100 ~ 2,
        TRUE ~ NA_real_),

      # isaric_cox_average_nlr = case_when(
      #   !! .nlr <= 6 ~ 0,
      #   !! .nlr >  6 ~ 1,
      #   TRUE ~ NA_real_),
      #
      # isaric_cox_average_plts = case_when(
      #   !! .plts < 100 ~ 2,
      #   !! .plts >=100 ~ 0,
      #   TRUE ~ NA_real_),
      #
      # isaric_cox_average_creat = case_when(
      #   !! .creat < 100 ~ 0,
      #   !! .creat >= 100 ~ 2,
      #   TRUE ~ NA_real_),
    ...
    mutate(
      isaric_cox_average = rowSums(dplyr::select(., dplyr::starts_with("isaric_cox_average_")),
                                   na.rm = na_to_zeros)
    ) %>%

    {if(all_na_rm){
      dplyr::mutate(., isaric_cox_average = dplyr::if_else(
        is.na(isaric_cox_average_age) &
          is.na(isaric_cox_average_sex) &
          is.na(isaric_cox_average_comorbid) &
          is.na(isaric_cox_average_rr) &
          is.na(isaric_cox_average_spo2) &
          is.na(isaric_cox_average_gcs) &
          # is.na(isaric_cox_average_nlr) &
          # is.na(isaric_cox_average_plts) &
          # is.na(isaric_cox_average_hb) &
          is.na(isaric_cox_average_bun) &
          # is.na(isaric_cox_average_creat) &
          # is.na(isaric_cox_average_lactate),
          is.na(isaric_cox_average_crp), NA_real_, isaric_cox_average))
    } else {
      .
    }}
  ...
  if(output == "vector"){
    out %>%
      pull(isaric_cox_average)
  } else if(output == "components"){
    out %>%
      select(starts_with("isaric_cox_average"))
  } else if(output == "df_vector"){
    out %>%
      pull(isaric_cox_average) %>%
      bind_cols(.data, isaric_cox_average = .)
  } else if(output == "df_components"){
    out
  }
}
...
# Isaric full score -------------------------------------------------------------------------------------------
#
# isaric <- function(.data, age, sex, comorbid, rr, spo2, sbp, gcs,
#                    nlr, plts, hb, bun, creat, crp, lactate,
#                    output = c("vector", "components", "df_vector", "df_components"),
#                    na_to_zeros = TRUE, all_na_rm = TRUE){
#
...
#
#   out = .data %>%
#     dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .spo2, !! .sbp,
#                           !! .gcs, !! .nlr, !! .plts, !! .hb, !! .bun,
#                           !! .creat, !! .crp, !! .lactate), as.numeric) %>%
#     dplyr::mutate_at(vars(!! .sex), ~ as.character(.) %>% tolower() %>% trimws()) %>%
#
#     mutate(
#       isaric_age = case_when(
#         !! .age < 50 ~ 0,
#         !! .age < 60 ~ 2,
#         !! .age < 70 ~ 3,
#         !! .age < 80 ~ 6,
...
#         TRUE ~ NA_real_),
#
#       isaric_sex = case_when(
#         !! .sex == "male" ~ 1,
#         !! .sex == "female" ~ 0,
#         TRUE ~ NA_real_),
#
#       isaric_comorbid = case_when(
#         !! .comorbid == "0" ~ 0,
#         !! .comorbid == "1" ~ 1,
#         !! .comorbid >= "2" ~ 3,
#         TRUE ~ NA_real_),
...
#         TRUE ~ NA_real_),
#
#       isaric_spo2 = case_when(
#         !! .spo2 < 90 ~ 5,
#         !! .spo2 >= 90 ~ 0,
#         TRUE ~ NA_real_),
#
#       isaric_sbp = case_when(
#         !! .sbp <  90 ~ 2,
#         !! .sbp >= 90 ~ 0,
#         TRUE ~ NA_real_),
#
#       isaric_gcs = case_when(
#         !! .gcs <=  14 ~ 5,
#         !! .gcs > 14 ~ 0,
#         TRUE ~ NA_real_),
#
#       isaric_nlr = case_when(
#         !! .nlr <= 6 ~ 0,
#         !! .nlr >  6 ~ 1,
#         TRUE ~ NA_real_),
#
#       isaric_plts = case_when(
#         !! .plts < 100 ~ 3,
#         !! .plts >=100 ~ 0,
#         TRUE ~ NA_real_),
#
...
#         TRUE ~ NA_real_),
#
#       isaric_bun = case_when(
#         !! .bun <= 7 ~ 0,
#         !! .bun >  7 ~ 2,
#         TRUE ~ NA_real_),
#
#       isaric_creat = case_when(
#         !! .creat < 100 ~ 0,
#         !! .creat >= 100 ~ 2,
#         TRUE ~ NA_real_),
#
#       isaric_crp = case_when(
#         !! .crp < 100 ~ 0,
#         !! .crp >=100 ~ 3,
#         TRUE ~ NA_real_),
#
#       isaric_lactate = case_when(
#         !! .lactate < 2 ~ 0,
#         !! .lactate >=2 ~ 2,
#         TRUE ~ NA_real_)
#
#     ) %>%
#     mutate(
#       isaric = rowSums(dplyr::select(., dplyr::starts_with("isaric_")),
#                        na.rm = na_to_zeros)
#     ) %>%
#
#     {if(all_na_rm){
#       dplyr::mutate(., isaric = dplyr::if_else(
#         is.na(isaric_age) &
#           is.na(isaric_sex) &
#           is.na(isaric_comorbid) &
#           is.na(isaric_rr) &
#           is.na(isaric_spo2) &
#           is.na(isaric_sbp) &
#           is.na(isaric_gcs) &
...
#           is.na(isaric_creat) &
#           is.na(isaric_crp) &
#           is.na(isaric_lactate), NA_real_, isaric))
#     } else {
#       .
#     }}
#
...
#   } else if(output == "components"){
#     out %>%
#       select(starts_with("isaric"))
#   } else if(output == "df_vector"){
#     out %>%
#       pull(isaric) %>%
#       bind_cols(.data, isaric = .)
#   } else if(output == "df_components"){
#     out
#   }
# }
#
#
# # Isaric reduced score -------------------------------------------------------------------------------------------
# isaricR <- function(.data, age, sex, comorbid, rr, spo2, sbp, gcs, plts, crp, bun, nlr,
#                     # nlr, plts, hb, bun, creat, crp, lactate,
#                     output = c("vector", "components", "df_vector", "df_components"),
#                     na_to_zeros = TRUE, all_na_rm = TRUE){
#
...
#
#   out = .data %>%
#     dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .spo2, !! .sbp, !! .gcs, !! .plts, !! .crp, !! .bun, !!.nlr
#                           # !! .nlr, !! .plts, !! .hb, !! .bun, !! .creat, !! .crp, !! .lactate
#     ), as.numeric) %>%
#     dplyr::mutate_at(vars(!! .sex), ~ as.character(.) %>% tolower() %>% trimws()) %>%
#
#     mutate(
#       isaricR_age = case_when(
#         !! .age < 50 ~ 0,
#         !! .age < 60 ~ 2,
#         !! .age < 70 ~ 3,
#         !! .age < 80 ~ 6,
...
#         TRUE ~ NA_real_),
#
#       isaricR_sex = case_when(
#         !! .sex == "male" ~ 1,
#         !! .sex == "female" ~ 0,
#         TRUE ~ NA_real_),
#
#       isaricR_comorbid = case_when(
#         !! .comorbid == "0" ~ 0,
#         !! .comorbid == "1" ~ 1,
#         !! .comorbid >= "2" ~ 3,
#         TRUE ~ NA_real_),
#
#       isaricR_rr = case_when(
#         !! .rr < 20 ~ 0,
#         !! .rr < 30 ~ 2,
#         !! .rr >= 30 ~ 4,
#         TRUE ~ NA_real_),
#
#       isaricR_spo2 = case_when(
#         !! .spo2 < 90 ~ 5,
#         !! .spo2 >= 90 ~ 0,
#         TRUE ~ NA_real_),
#
#       isaricR_sbp = case_when(
#         !! .sbp <  90 ~ 2,
#         !! .sbp >= 90 ~ 0,
#         TRUE ~ NA_real_),
#
#       isaricR_gcs = case_when(
#         !! .gcs <=  14 ~ 5,
#         !! .gcs > 14 ~ 0,
#         TRUE ~ NA_real_),
#
#       isaricR_nlr = case_when(
#         !! .nlr <= 6 ~ 0,
#         !! .nlr >  6 ~ 1,
#         TRUE ~ NA_real_),
#
#       isaricR_plts = case_when(
#         !! .plts < 100 ~ 3,
#         !! .plts >=100 ~ 0,
#         TRUE ~ NA_real_),
#       #
#       # isaricR_hb = case_when(
#       #   !! .hb < 120 ~ 2,
#       #   !! .hb >= 120 ~ 0,
#       #   TRUE ~ NA_real_),
#       #
#       isaricR_bun = case_when(
#         !! .bun <= 7 ~ 0,
#         !! .bun >  7 ~ 2,
#         TRUE ~ NA_real_),
#       #
#       # isaricR_creat = case_when(
#       #   !! .creat < 100 ~ 0,
#       #   !! .creat >= 100 ~ 2,
#       #   TRUE ~ NA_real_),
#       #
#       isaricR_crp = case_when(
#         !! .crp < 100 ~ 0,
#         !! .crp >=100 ~ 3,
#         TRUE ~ NA_real_)
#       #
#       # isaricR_lactate = case_when(
#       #   !! .lactate < 2 ~ 0,
#       #   !! .lactate >=2 ~ 2,
#       #   TRUE ~ NA_real_)
#
#     ) %>%
#     mutate(
#       isaricR = rowSums(dplyr::select(., dplyr::starts_with("isaricR_")),
#                         na.rm = na_to_zeros)
#     ) %>%
#
#     {if(all_na_rm){
#       dplyr::mutate(., isaricR = dplyr::if_else(
#         is.na(isaricR_age) &
#           is.na(isaricR_sex) &
#           is.na(isaricR_comorbid) &
#           is.na(isaricR_rr) &
#           is.na(isaricR_spo2) &
#           is.na(isaricR_sbp) &
#           is.na(isaricR_gcs) &
...
#           is.na(isaricR_bun) &
#           # is.na(isaricR_creat) &
#           # is.na(isaricR_lactate),
#           is.na(isaricR_crp), NA_real_, isaricR))
#     } else {
#       .
#     }}
#
...
#   } else if(output == "components"){
#     out %>%
#       select(starts_with("isaricR"))
#   } else if(output == "df_vector"){
#     out %>%
#       pull(isaricR) %>%
#       bind_cols(.data, isaricR = .)
#   } else if(output == "df_components"){
#     out
#   }
# }
#
# ### Reduced score (exclude nlr, plts, creat) ############################################
# lasso_score_reduced <- function(.data, age, sex, comorbid, rr, spo2, hr, sbp, gcs, infiltrates,
#                              # plts, nlr, creat,
#                              bun, crp,
#                              output = c("vector", "components", "df_vector", "df_components")){
#
...
#     mutate(
#       # Lasso full score
#       lasso_score_reduced_age = !! .age * 0.5390,
#       lasso_score_reduced_comorbid = !! .comorbid * 0.1703,
#       lasso_score_reduced_rr = !! .rr * 0.0201,
#       lasso_score_reduced_spo2 = !! .spo2 * -0.0539,
#       lasso_score_reduced_hr = !! .hr * 0.0044,
#       lasso_score_reduced_sbp = !! .sbp * -0.0023,
#       lasso_score_reduced_gcs = !! .gcs * -0.0958,
#       lasso_score_reduced_bun = !! .bun * 0.0223,
#       lasso_score_reduced_crp = !! .crp * 0.0021,
#
#       lasso_score_reduced_sex = case_when(
#         !! .sex == "male" ~ 0.0371,
#         is.na(!! .sex) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_score_reduced_infiltrates = case_when(
#         !! .infiltrates == "Yes" ~ 0.0310,
#         is.na(!! .infiltrates) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_score_reduced_link = 0.8557 + lasso_score_reduced_age + lasso_score_reduced_sex + lasso_score_reduced_comorbid + lasso_score_reduced_rr +
#         lasso_score_reduced_spo2 + lasso_score_reduced_hr + lasso_score_reduced_sbp + lasso_score_reduced_gcs +
#         lasso_score_reduced_bun + lasso_score_reduced_crp + lasso_score_reduced_infiltrates,
#       lasso_score_reduced = boot::inv.logit((lasso_score_reduced_link))
#     )
#
#   if(output == "vector"){
#     out %>%
#       pull(lasso_score_reduced)
#   } else if(output == "components"){
#     out %>%
#       select(starts_with("lasso_score_reduced"))
#   } else if(output == "df_vector"){
#     out %>%
#       pull(lasso_score_reduced) %>%
#       bind_cols(.data, lasso_score_reduced = .)
#   } else if(output == "df_components"){
#     out
#   }
# }
...
#
# ### Lasso cat ############################################
# lasso_cat <- function(.data, age, sex, comorbid, rr, spo2, hr, gcs, dbp, infiltrates,
#                                 plts, nlr, sodium, bun, creat, crp,
#                                 output = c("vector", "components", "df_vector", "df_components")){
#
#
#   .age = enquo(age)
...
#   out = .data %>%
#     dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .spo2,
#                           !! .hr, !! .dbp, !! .gcs, !! .bun, !! .crp, !! .sodium, !! .plts, !! .nlr, !! .creat
#     ), as.numeric) %>%
#     dplyr::mutate_at(vars(!! .sex, !! .infiltrates), ~ as.character(.) %>% tolower() %>% trimws()) %>%
#
#     mutate(
#       lasso_cat_age = case_when(
#         !! .age <60 ~ 0,
#         !! .age <70 ~ age * 0.5267,
#         !! .age <80 ~ age * 1.0533,
#         !! .age >=80 ~ age * 1.5307,
...
#         TRUE ~ 0),
#
#       lasso_cat_comorbid = case_when(
#         !! .comorbid == "0"~ 0,
#         !! .comorbid == "1" ~ 0.1377,
#         !! .comorbid >= "2" ~ !! .comorbid * 0.4115,
#         is.na(!! .comorbid) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_rr = case_when(
#         !! .rr < 20 ~ 0,
#         !! .rr < 30 ~ !! .rr * 0.0340,
#         !! .rr >= 30 ~ !! .rr * 0.3594,
#         is.na(!! .rr) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_spo2 = case_when(
#         !! .spo2 >= 90 ~ !! .spo2 * -0.7633,
#         is.na(!! .spo2) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_hr = case_when(
#         !! .hr > 100 ~ !! .hr * 0.0490,
#         is.na(!! .hr) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_dbp = case_when(
#         !! .dbp >= 60 ~ !! .dbp * -0.2399,
#         is.na(!! .dbp) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_gcs = case_when(
#         !! .gcs > 14 ~ !! .gcs * 0.6424,
#         is.na(!! .gcs) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_plts = case_when(
#         !! .plts >= 100 ~ !! .plts * -0.5401,
#         is.na(!! .plts) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_nlr = case_when(
#         !! .nlr >= 6 ~ !! .nlr * 0.2679,
#         is.na(!! .nlr) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_sodium = case_when(
#         !! .sodium <130 ~ 0,
#         !! .sodium <=145 ~ !! .sodium * -0.2443,
#         !! .sodium >145 ~ !! .sodium * 0.4362,
#         is.na(!! .sodium) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_bun = case_when(
#         !! .bun > 7 ~ !! .bun * 0.5612,
#         is.na(!! .bun) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_creat = case_when(
#         !! .creat < 100 ~ !! .creat * -0.2226,
#         is.na(!! .creat) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_crp = case_when(
#         !! .crp > 100 ~ !! .crp * 0.4248,
#         is.na(!! .crp) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_sex = case_when(
#         !! .sex == "male" ~ 0.0515,
#         is.na(!! .sex) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_infiltrates = case_when(
#         !! .infiltrates == "Yes" ~   0.1469,
#         is.na(!! .infiltrates) ~ NA_real_,
#         TRUE ~ 0),
#
#       lasso_cat_link = -1.0270 + lasso_cat_age + lasso_cat_sex + lasso_cat_comorbid + lasso_cat_rr +
#         lasso_cat_spo2 + lasso_cat_hr + lasso_cat_dbp, lasso_cat_gcs + lasso_cat_sodium + lasso_cat_plts +
#         lasso_cat_bun + lasso_cat_crp + lasso_cat_nlr + lasso_cat_infiltrates,
#       lasso_cat = boot::inv.logit((lasso_cat_link))
#     )
#
#   if(output == "vector"){
#     out %>%
...
#   } else if(output == "components"){
#     out %>%
#       select(starts_with("lasso_cat"))
#   } else if(output == "df_vector"){
#     out %>%
#       pull(lasso_cat) %>%
#       bind_cols(.data, lasso_cat = .)
#   } else if(output == "df_components"){
#     out
#   }
# }
...
#
# # isaric_min_lasso
# isaric_min_lasso <- function(.data, age, sex, comorbid, rr, dbp,
#                              spo2, gcs, crp, bun, infiltrates,
#                              # nlr, plts, hb, bun, creat, crp, lactate,
#                              output = c("vector", "components", "df_vector", "df_components"),
#                              na_to_zeros = TRUE, all_na_rm = TRUE){
#
...
#
#   out = .data %>%
#     dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .dbp,
#                           !! .spo2, !! .gcs, !! .crp, !! .bun,
#                           # !! .nlr, !! .plts, !! .hb, !! .bun, !! .creat, !! .crp, !! .lactate
#     ), as.numeric) %>%
#     dplyr::mutate_at(vars(!! .sex, !! .infiltrates), ~ as.character(.) %>% tolower() %>% trimws()) %>%
#
#     mutate(
#       isaric_min_lasso_age = case_when(
#         !! .age < 60 ~ 0,
#         !! .age < 70 ~ 3,
#         !! .age < 80 ~ 6,
#         !! .age >= 80 ~ 8,
#         TRUE ~ NA_real_),
#
#       isaric_min_lasso_sex = case_when(
#         !! .sex == "male" ~ 1,
#         !! .sex == "female" ~ 0,
#         TRUE ~ NA_real_),
#
#       isaric_min_lasso_comorbid = case_when(
#         !! .comorbid == "0" ~ 0,
#         !! .comorbid == "1" ~ 1,
#         !! .comorbid >= "2" ~ 3,
#         TRUE ~ NA_real_),
#
#       isaric_min_lasso_rr = case_when(
#         !! .rr < 20 ~ 0,
#         !! .rr < 30 ~ 1,
#         !! .rr >= 30 ~ 2,
#         TRUE ~ NA_real_),
#
#       isaric_min_lasso_spo2 = case_when(
#         !! .spo2 < 90 ~ 4,
#         !! .spo2 >= 90 ~ 0,
#         TRUE ~ NA_real_),
#
#       lasso_min_lasso_dbp = case_when(
#         !! .dbp < 60 ~ 1,
#         !! .dbp >= 60 ~ 0,
#         TRUE ~ NA_real_),
#
#       isaric_min_lasso_gcs = case_when(
#         !! .gcs <=  14 ~ 4,
#         !! .gcs > 14 ~ 0,
#         TRUE ~ NA_real_),
#
#       # isaric_min_lasso_nlr = case_when(
#       #   !! .nlr <= 6 ~ 0,
#       #   !! .nlr >  6 ~ 1,
#       #   TRUE ~ NA_real_),
#
#       # isaric_min_lasso_plts = case_when(
#       #   !! .plts < 100 ~ 3,
#       #   !! .plts >=100 ~ 0,
#       #   TRUE ~ NA_real_),
#       #
#       # isaric_min_lasso_hb = case_when(
#       #   !! .hb < 120 ~ 2,
#       #   !! .hb >= 120 ~ 0,
#       #   TRUE ~ NA_real_),
#       #
#       isaric_min_lasso_bun = case_when(
#         !! .bun <= 7 ~ 0,
#         !! .bun >  7 ~ 3,
#         TRUE ~ NA_real_),
#       #
#       # isaric_min_lasso_creat = case_when(
#       #   !! .creat < 100 ~ 0,
#       #   !! .creat >= 100 ~ 2,
#       #   TRUE ~ NA_real_),
#       #
#       isaric_min_lasso_crp = case_when(
#         !! .crp < 100 ~ 0,
#         !! .crp >=100 ~ 3,
#         TRUE ~ NA_real_),
#
#       isaric_min_lasso_infiltrates = case_when(
#         !! .infiltrates == "Yes" ~ 1,
#         !! .infiltrates == "No" ~ 0,
#         TRUE ~ NA_real_),
#       #
#       # isaric_min_lasso_lactate = case_when(
#       #   !! .lactate < 2 ~ 0,
#       #   !! .lactate >=2 ~ 2,
#       #   TRUE ~ NA_real_)
#
#     ) %>%
#     mutate(
#       isaric_min_lasso = rowSums(dplyr::select(., dplyr::starts_with("isaric_min_lasso_")),
#                            na.rm = na_to_zeros)
#     ) %>%
#
#     {if(all_na_rm){
#       dplyr::mutate(., isaric_min_lasso = dplyr::if_else(
#         is.na(isaric_min_lasso_age) &
#           is.na(isaric_min_lasso_sex) &
#           is.na(isaric_min_lasso_comorbid) &
#           is.na(isaric_min_lasso_rr) &
#           is.na(lasso_min_lasso_dbp) &
#           is.na(isaric_min_lasso_spo2) &
#           is.na(isaric_min_lasso_gcs) &
#           # is.na(isaric_min_lasso_nlr) &
#           # is.na(isaric_min_lasso_plts) &
#           # is.na(isaric_min_lasso_hb) &
#           is.na(isaric_min_lasso_bun) &
#           # is.na(isaric_min_lasso_creat) &
#           # is.na(isaric_min_lasso_lactate),
#           is.na(isaric_min_lasso_infiltrates) &
#           is.na(isaric_min_lasso_crp), NA_real_, isaric_min_lasso))
#     } else {
#       .
#     }}
#
#   if(output == "vector"){
#     out %>%
#       pull(isaric_min_lasso)
#   } else if(output == "components"){
#     out %>%
#       select(starts_with("isaric_min_lasso"))
#   } else if(output == "df_vector"){
#     out %>%
#       pull(isaric_min_lasso) %>%
#       bind_cols(.data, isaric_min_lasso = .)
#   } else if(output == "df_components"){
#     out
#   }
# }
...
#
#
# lasso_score_full(age = age, sex = sex, comorbid = no_comorbid, rr = rr_vsorres,
#                  spo2 = oxy_vsorres, hr = hr_vsorres, sbp = sysbp_vsorres,
#                  gcs = daily_gcs_vsorres, infiltrates = infiltrates_faorres,
#                  bun = daily_bun_lborres, crp = daily_crp_lborres, creat = daily_creat_lborres,
#                  nlr = NLR, plts = daily_plt_lborres,
#                  output = c("df_vector")) %>%
#   lasso_score_full2(age = age, sex = sex, comorbid = no_comorbid, rr = rr_vsorres,
#                     spo2 = oxy_vsorres, hr = hr_vsorres, sbp = sysbp_vsorres, dbp = admission_diabp_vsorres,
#                     gcs = daily_gcs_vsorres, infiltrates = infiltrates_faorres,
#                     bun = daily_bun_lborres, crp = daily_crp_lborres, creat = daily_creat_lborres,
#                     nlr = NLR, plts = daily_plt_lborres,
#                     output = c("df_vector")) %>%
#   lasso_score_reduced(age = age, sex = sex, comorbid = no_comorbid, rr = rr_vsorres,
#                       spo2 = oxy_vsorres, hr = hr_vsorres,  sbp = sysbp_vsorres,
#                       gcs = daily_gcs_vsorres, infiltrates = infiltrates_faorres,
#                       bun = daily_bun_lborres, crp = daily_crp_lborres,
#                       output = c("df_vector")) %>%
#   lasso_cat(age = age, sex = sex, comorbid = no_comorbid, rr = rr_vsorres,
#             spo2 = oxy_vsorres, hr = hr_vsorres, dbp = admission_diabp_vsorres,
#             gcs = daily_gcs_vsorres, infiltrates = infiltrates_faorres, sodium = daily_sodium_lborres,
#             bun = daily_bun_lborres, crp = daily_crp_lborres, creat = daily_creat_lborres,
#             nlr = NLR, plts = daily_plt_lborres,
#             output = c("df_vector")) %>%
#   isaric(age = age, sex = sex, comorbid = no_comorbid, rr = rr_vsorres,
#          spo2 = oxy_vsorres, sbp = sysbp_vsorres,
#          gcs = daily_gcs_vsorres, nlr = NLR, plts = daily_plt_lborres, hb = daily_hb_lborres,
#          bun = daily_bun_lborres, crp = daily_crp_lborres, creat = daily_creat_lborres,
#          lactate =daily_lactate_lborres,
#          output = c("df_vector"), na_to_zeros = FALSE) %>%
#   isaricR(age = age, sex = sex, comorbid = no_comorbid, rr = rr_vsorres,
#           spo2 = oxy_vsorres, sbp = sysbp_vsorres,
#           gcs = daily_gcs_vsorres, nlr = NLR, plts = daily_plt_lborres,
#           bun = daily_bun_lborres, crp = daily_crp_lborres,
#           output = c("df_vector"), na_to_zeros = FALSE)



# Old clinical version

# isaric_clin <- function(.data, age, sex, comorbid, rr, spo2, sbp, gcs, # crp, bun,
#                         # nlr, plts, hb, bun, creat, crp, lactate,
#                         output = c("vector", "components", "df_vector", "df_components"),
#                         na_to_zeros = TRUE, all_na_rm = TRUE){
#
...
#
#   out = .data %>%
#     dplyr::mutate_at(vars(!! .age, !! .comorbid, !! .rr, !! .spo2, !! .sbp, !! .gcs, #!! .crp, !! .bun,
#                           # !! .nlr, !! .plts, !! .hb, !! .bun, !! .creat, !! .crp, !! .lactate
#     ), as.numeric) %>%
#     dplyr::mutate_at(vars(!! .sex), ~ as.character(.) %>% tolower() %>% trimws()) %>%
#
...
#
# Old minimal model
# isaric_min <- function(.data, age, sex, comorbid, rr, spo2, sbp, gcs, crp, bun,
#                        # nlr, plts, hb, bun, creat, crp, lactate,
#                        output = c("vector", "components", "df_vector", "df_components"),
#                        na_to_zeros = TRUE, all_na_rm = TRUE){
#
